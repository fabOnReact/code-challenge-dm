...
export function mergeList(list) {
  ...
  checkIntervalAndThrow(sortedList[0]);
  let mergedList = [sortedList[0]];
  for (let i = 1; i < sortedList.length; i++) {
    const currentInterval = list[i];
    checkIntervalAndThrow(currentInterval);
...
export function getMessage(type, data) {
  const ERROR_INTERVAL_START =
    "is not acceptable parameter. The first value (start minute) should be smaller then second value (end minute).";
  ...
  switch (type) {
    ...
    case "WRONG_INTERVAL_START":
      return `${ERROR_MSG} ${json} ${ERROR_INTERVAL_START}`;
    default:
      console.error(
        `type: ${type} with ${data} does not have a corresponding error in getErrorMessage.`
      );
  }
}
